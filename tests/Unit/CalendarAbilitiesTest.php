...
class CalendarAbilitiesTest extends WP_UnitTestCase {
	...
	public function test_title_search_preserves_explicit_location_and_past_date_scope(): void {
		$location       = wp_insert_term( 'Search location ' . uniqid(), 'calendar_test_region' );
		$other_location = wp_insert_term( 'Other search location ' . uniqid(), 'calendar_test_region' );
		...
		$this->seed_event(
			...
			$date->format( 'Y-m-d 21:00:00' ),
			0,
			array( 'calendar_test_region' => (int) $other_location['term_id'] )
		);
		$this->seed_event(
			$title,
			$other_date->format( 'Y-m-d 18:00:00' ),
			$other_date->format( 'Y-m-d 21:00:00' ),
			0,
			array( 'calendar_test_region' => (int) $location['term_id'] )
		);

		foreach ( array( $title, 'Flow Tribe', 'J & The Causeways', 'flow tribe' ) as $search ) {
			$result = $this->abilities->executeGetCalendarPage(
				array(
					'event_search'     => $search,
			...
			$this->assertSame( 1, $result['total_event_count'], $search );
			$this->assertSame( 1, $result['event_count'], $search );
			$this->assertSame( array( $target_id ), $this->result_post_ids( $result ), $search );
		}

		$no_match = $this->abilities->executeGetCalendarPage(
			array(
				'event_search'     => 'Genuine no match ' . uniqid(),
				'archive_taxonomy' => 'calendar_test_region',
				'archive_term_id'  => (int) $location['term_id'],
				'date_start'       => $date->format( 'Y-m-d' ),
				'date_end'         => $date->format( 'Y-m-d' ),
		...
		$this->assertSame( 0, $no_match['total_event_count'] );
		$this->assertSame( 0, $no_match['event_count'] );
		$this->assertSame( array(), $no_match['paged_date_groups'] );
	}
